...
export default function ViewComment({ comment, userId }) {
    ...
    return (
        <div className='comment'>
            ...
            <img className='profile-image' src={comment.User.pfp ? comment.User.pfp : 'https://media.istockphoto.com/id/1337144146/vector/default-avatar-profile-icon-vector.jpg?s=1024x1024&w=is&k=20&c=6XEZlH2FjqdpXUqjUK4y0LlWF6yViZVWn9HZJ-IR8gU='} alt='profile' />

            <div className='comment-body'>
                <div className='comment-container'>
                    <h3 className='comment-user-info'>{comment.User.firstName} {comment.User.lastName}</h3>
                    {
                        userId === comment.userId && <img className='comment-edit' src='/images/icons/edit.png' onClick={() => setEdit(!edit)} alt='edit-comment'/>
                    }
                </div>
                {
